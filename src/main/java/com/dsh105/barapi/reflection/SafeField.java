...
public class SafeField<T> implements FieldAccessor<T> {
    ...
    public SafeField(Class<?> coreClass, String fieldName) {
        try {
            ...
            BarAPI.getCore().LOGGER_REFLECTION.warning("Failed to find a matching field with name: " + fieldName);
            e.printStackTrace();
        }
    }
    ...
    public boolean set(Object instance, T value) {
        ...
        try {
            ...
            BarAPI.getCore().LOGGER_REFLECTION.warning("Failed to access field: " + toString());
            e.printStackTrace();
        }
        return false;
    }
