...
public class ClassTemplate<T> {
    ...
    public static ClassTemplate<?> create(Class<?> type) {
        if (type == null) {
            BarAPI.getCore().LOGGER_REFLECTION.warning("Cannot create a ClassTemplate with a null type!");
            return null;
        }
    ...
    public static ClassTemplate<?> create(String className) {
        ...
        if (clazz == null) {
            BarAPI.getCore().LOGGER_REFLECTION.warning("Failed to find a matching class with name: " + className);
            return null;
        }
